...
def check_model_file_from_stem(model="yolo11n"):
    ...
    参数:
        model (str): 要检查的模型名称（不带后缀）
    
    返回:
        (str | Path): 带有正确后缀的模型文件名。如果输入是有效的模型名称且无后缀，则添加'.pt'后缀；否则返回原输入。
    """
    """
...
def check_imshow(warn=False):
    ...
    try:
        ...
        cv2.imshow("our", np.zeros((8, 8, 3), dtype=np.uint8))  # show a small 8-pixel image
        cv2.waitKey(1)
