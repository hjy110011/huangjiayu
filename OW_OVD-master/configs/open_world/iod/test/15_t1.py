...
visdrone_class_names = [
    "pedestrian", "people", "bicycle", "car", "van"# 补全10类
]
class_names = visdrone_class_names

# --- 2. Step 1 IOD/开放世界设置 (5+5 划分) ---
VISDRONE_DATA_ROOT = '/home/gdut-627/huangjiayu/datasets/VisDrone' # VisDrone11 根目录
split = 5
prev_intro_cls = 0
cur_intro_cls = 5

# 假设 VisDrone11 转换后的 IOD 标注文件
# **注意：请确保 annotations/train_coco.json 是仅包含前 5 类的标注**
train_json = 'OWOD/VisDrone_OWOD_step1_5classes.json'
embedding_path = f'/home/gdut-627/huangjiayu/datasets/VisDrone11/OWOD/gt_5_t1.npy' # 5类文本嵌入
att_embeddings = f'/home/gdut-627/huangjiayu/datasets/VisDrone11/OWOD/att_embeddings.pth'
pipline = []

# --- 3. 模型和训练参数 ---
num_classes = 10 # 总类别数
num_training_classes = 5 # 当前训练类别数 (Step 1)
max_epochs = 30  # 示例值，请根据实验需要调整
...
model = dict(type='OurDetector',
             mm_neck=True,
             num_train_classes=num_training_classes, # 5
             ...
             bbox_head=dict(type='OurHead',
                            ...
                            prev_intro_cls=prev_intro_cls, # 0
                            ...
                            head_module=dict(
                                ...
                                num_classes=num_training_classes,),), # 5
             train_cfg=dict(assigner=dict(num_classes=num_training_classes))) # 5
...
coco_train_dataset = dict(
        ...
        dataset=dict(
            ...
            ann_file=train_json, # 5类 JSON 标注
            data_prefix=dict(img='VisDrone2019-DET-train/images/'),
            filter_cfg=dict(filter_empty_gt=False, min_size=4)),
        class_text_path=f'/home/gdut-627/huangjiayu/datasets/VisDrone11/visdrone_class_texts_5cls.json',
...
default_hooks = dict(
    ...
    param_scheduler=dict(
        lr_factor=0.01,
        max_epochs=500, # 基线配置的 max_epochs，不影响当前 max_epochs
...
test_dataloader = dict(dataset=dict(type='YOLOv5CocoDataset',
                        data_root=VISDRONE_DATA_ROOT,
                        ann_file='OWOD/VisDrone_val_step2_10classes.json', # 全部 10 类的测试集标注
...
test_evaluator = dict(_delete_=True,
                     ...
                     cfg=dict(
                        ...
                        file_name='annotations/our.txt', # 评估需要的文件
                        prev_intro_cls=prev_intro_cls, # 0
                        cur_intro_cls=cur_intro_cls, # 5
                        unknown_id=5, # 在只有 10 类的情况下，通常设置为 10 或 11
                        class_names=visdrone_class_names
                     )
                    )
