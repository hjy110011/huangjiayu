...
class FOMOnoBNHeadModule(YOLOv8HeadModule):
...
class FOMOnoBNHead(YOLOv8Head):
    ...
    def __init__(self, world_size=-1, 
        ...
        self.collect_embedding = None
    
    def set_collect(self, mode):
        if mode:
            self.collect = True
            if self.collect_embedding is None:
                self.collect_embedding = {i: (0,0) for i in range(self.num_classes)}
        else:
            self.collect = False
    ...
    def apply_linear(self, outs):
        ...
        for cls_out in outs[0]:
            ...
            if self.training:
                ret_outs.append((cls_pred.permute(0,3,1,2), image_feat))
            else:
                ret_outs.append(cls_pred.permute(0,3,1,2))
    ...
    def predict_known_unknown(self, outs):
        ...
        for att, score in zip(att_scores, scores):
            ...
            att = att.permute(0,2,3,1).sigmoid()
            ...
            if score.shape[-1] == self.num_classes:
                ...
                obj = score[..., -1][..., None]
            id_prob = att.max(dim=-1, keepdim=True)[0]
            ood_prob = (1 - known.max(dim=-1, keepdim=True)[0])
            unknown_score = (id_prob * ood_prob)
            unknown_score -= unknown_score.mean()
            unknown_score = unknown_score / unknown_score.std()
            unknown_score = (unknown_score) * obj
    ...
    @torch.no_grad()
    def collect_mean_embeddings(self, img_feats: Tensor, 
                                    assigned_scores: Tensor,
                                    thr=0.8):
        """Collect mean embeddings based on thresholded assigned scores.

        Args:
            img_feats (Tensor): (bs, -1, embedding_dim)
            assigned_scores (Tensor): (bs, -1, num_classes)
        """
        if not self.collect:
            return

        # Reshape input tensors
        embedding_dim = img_feats.shape[-1]
        img_feats = img_feats.view(-1, embedding_dim)
        assigned_scores = assigned_scores.view(-1, self.num_classes)

        for i in range(self.num_classes):
            mask = assigned_scores[..., i] > thr
            mask_sum = mask.sum()
            
            if mask_sum > 0:
                # Accumulate embeddings and counts
                masked_feats_sum = img_feats[mask].sum(dim=0)
                self.collect_embedding[i] = (
                    self.collect_embedding[i][0] + masked_feats_sum,
                    self.collect_embedding[i][1] + mask_sum
                )

    def loss_by_feat(
            self,
            cls_scores: Sequence[tuple],    # (cls_pred, image_feat)    
        ...
        feats = [feat for _, feat in cls_scores]
        cls_scores = [cls_score for cls_score, _ in cls_scores]
        ...
        flatten_feats = [
            feat.permute(0, 2, 3, 1).reshape(num_imgs, -1, 512)     # 512 is the feature dim
            for feat in feats
        ]
        ...
        # (bs, n, 512)
        flatten_feats = torch.cat(flatten_feats, dim=1)         
        ...
        self.collect_mean_embeddings(flatten_feats, assigned_scores)
