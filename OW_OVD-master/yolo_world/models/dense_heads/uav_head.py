...
class UavHead(YOLOv8Head):
    ...
    def loss(self, img_feats: Tuple[Tensor], txt_feats: Tensor,
        ...
        if self.att_embeddings is None:
            ...
            # [修改 2] 传递 epoch_info 到 loss_by_feat
        ...
        loss_inputs = outs + (att_outs, batch_data_samples['bboxes_labels'],
                              batch_data_samples['img_metas'])
        # [修改 3] 传递 epoch_info 到 loss_by_feat
        losses = self.loss_by_feat(*loss_inputs, epoch_info=epoch_info)
    ...
    def loss_by_feat(
        ...
        # =================================================
        # Class-Balanced Loss (EMA + Effective Number)
        # =================================================
        # -------------------- Class Balanced Loss --------------------
        with torch.no_grad():
        ...
        if fg_mask.sum() > 0:
            ...
            pred_pos = torch.masked_select(
                flatten_pred_bboxes, prior_mask).reshape(-1, 4)
            gt_pos = torch.masked_select(
                assigned_bboxes, prior_mask).reshape(-1, 4)

            base_weight = torch.masked_select(
            ...
            assigned_ltrb_pos = torch.masked_select(
                assigned_ltrb, prior_mask).reshape(-1, 4)
    ...
    #         progress = min(max(cur_e / float(max_e), 0.0), 1.0)
    #
    #     # curriculum on cls head（不是 ProgLoss 本体，但合理）
    #     cls_weight = 1.5 - progress
