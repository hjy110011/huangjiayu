...
class UavBHead(YOLOv8Head):
    ...
    def loss_by_feat(
        ...
        if fg_mask.sum() > 0:
            ...
            wiou_loss = self.compute_wiou_loss(pred_pos, gt_pos, iou=ious)
            loss_bbox_wiou = (wiou_loss * (base_weight * prog_weight.unsqueeze(-1)).squeeze(
                -1)).sum() / assigned_scores_sum
            ...
            loss_bbox = loss_bbox_wiou + self.nwd_factor * loss_nwd
    ...
    def compute_wiou_loss(self, pred_bboxes, gt_bboxes, iou=None):
        ...
        # 3. 计算距离惩罚项 (Distance Attention) R_WIoU
        # 防止除以0
        cw = cw.clamp(min=1e-6)
        ch = ch.clamp(min=1e-6)

        dist_sq = (pred_cx - gt_cx) ** 2 + (pred_cy - gt_cy) ** 2
        diag_sq = cw ** 2 + ch ** 2

        # WIoU v1 的惩罚项: exp(dist / diag)
        R_WIoU = torch.exp(dist_sq / diag_sq)

        # 4. 构造非单调聚焦系数 (Non-monotonic Focusing) - WIoU v3 核心
        # beta: 离群度 outlier degree
        # loss_iou = 1 - IoU
        loss_iou = 1.0 - iou
        dist_attn = R_WIoU

        # beta = loss_iou * dist_attn / loss_iou.detach()
        # 简化版实现，直接用 IoU * R
        # 官方公式: beta = L_IoU * R_WIoU
        beta = loss_iou * R_WIoU

        # alpha, delta 是超参数，论文推荐 alpha=1.9, delta=3
        alpha = 1.9
        delta = 3.0

        # r: gradient gain
        # r = beta / (delta * alpha^(beta - delta))
        # 这是一个 "钟形" 曲线，抑制极大和极小的 beta
        r = beta / (delta * torch.pow(alpha, beta - delta))

        # 最终 Loss = r * L_IoU
        # 注意：r 应该 detach，不传导梯度给 r 的计算部分，只作为权重
