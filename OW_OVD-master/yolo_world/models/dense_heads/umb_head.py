...
class BNContrastiveHead(BaseModule):
    ...
    def forward(self, x: Tensor, w: Tensor) -> Tensor:
        ...
        scaled_log = x * self.logit_scale.exp() + self.bias
        return (scaled_log, x)
...
class UMBHeadModule(YOLOv8HeadModule):
...
@MODELS.register_module()
class UMBHead(YOLOv8Head):
    """YOLO-World Head
    """

    def __init__(self, world_size=-1, class_distribution_cfg=None,
                 *args, **kwargs) -> None:
        """_summary_

        Args:
            class_distribution_cfg: 
            {
                'save_root': str,
            }
        """
        ...
        self.class_distribution_cfg = class_distribution_cfg
        self.class_distribution = ClassDistribution(class_distribution_cfg)
        
    def set_linear(self, weight):
        self.att_linear = nn.Linear(weight['weight'].shape[1], weight['weight'].shape[0], bias=False)
        self.att_linear.load_state_dict(weight)
        self.att_linear = self.att_linear.to(weight['weight'].device)

    def set_vaild(self, valid):
        self.valid = valid
    ...
    def apply_linear(self, outs):
        ret_outs = []
        for cls_out in outs[0]:
            cls_out, sim = cls_out
            num_att = cls_out.shape[1]
            if num_att == self.att_linear.weight.shape[1]:
                cls_pred = self.att_linear(cls_out.permute(0,2,3,1))
            else:
                cls_pred = torch.cat([self.att_linear(cls_out.permute(0,2,3,1)[..., :-1]), 
                                      cls_out.permute(0,2,3,1)[..., -2:-1]], dim=-1)
            # set the cls_pred of invalid classes to 1e-6
            cls_pred[..., (~self.valid.bool())] = -65504   
            cls_pred = cls_pred.permute(0,3,1,2)      
            self.collect_sim(sim, cls_pred)
            ret_outs.append(cls_pred)
        return (ret_outs, *outs[1:])
    ...
    def predict(self,
        ...
        outs = self(img_feats, txt_feats)
        outs = self.predict_known_unknown(outs)
    ...
    def predict_known_unknown(self, outs):
        att_scores = outs[0]
        outs = self.apply_linear(outs)
        scores = outs[0]
        predictions = []
        for att, score in zip(att_scores, scores):
            score = score.permute(0,2,3,1).sigmoid()
            att = att[0].permute(0,2,3,1).sigmoid()
            known = score[..., :self.num_classes]
            # without obj
            if score.shape[-1] == self.num_classes:
                obj = torch.ones_like(known)[..., -1][..., None]
            else:
                obj = score[..., -1][..., None]
            id_prob = self.class_distribution.unknown_prediction(att)
            ood_prob = (1 - known.max(dim=-1, keepdim=True)[0])
            # unknown_score = (id_prob * ood_prob)
            unknown_score = (id_prob * ood_prob) * obj
            predictions.append(torch.cat([known, unknown_score], dim=-1).permute(0, 3, 1, 2))
        return (predictions, *outs[1:])

    def collect_sim(self, sim, cls_pred):
        if not self.training: 
            return
        
        if not os.path.exists(self.class_distribution_cfg['save_root']):
            os.makedirs(self.class_distribution_cfg['save_root']) 
        
        if 'log_sim' not in self.class_distribution_cfg:
            self.class_distribution_cfg['log_sim'] = []
        # due to scale
        sim = sim.sigmoid().detach().permute(0, 2, 3, 1).cpu()
        cls_pred = cls_pred.detach().permute(0, 2, 3, 1)[..., self.valid.bool()].cpu()
        torch.save({
            'logits': cls_pred,
            'cos_sim': sim,
        }, f'{self.class_distribution_cfg["save_root"]}/{len(self.class_distribution_cfg["log_sim"])+1}.pth')
        self.class_distribution_cfg['log_sim'].append(f'{self.class_distribution_cfg["save_root"]}/{len(self.class_distribution_cfg["log_sim"])+1}.pth')
...
class ClassDistribution():
    def __init__(self, args, mean_interval=0.0001, device='cuda'):
        """_summary_
            args:{
                'known_num': int,
                'att_w': Tensor, 
        ...
        self.category_num = args['known_num'] # unknown
        self.mean_interval = mean_interval
        self.args = args
        self.balance = args['balance']
        self.alpha = args['alpha']
        self.class_att_w = args['att_w']    # num_att, num_classes
        self.distributions = args['distribution']
        self.unknown_att_w = self.unknown_balance()
        self.device = device
    
    def set_att_w(self, att_w):
        self.class_att_w = att_w
        self.unknown_att_w = self.unknown_balance()
        
    def set_distribution(self, distributions):
        self.distributions = torch.Tensor(distributions).to(self.device)
        
    def unknown_balance(self):
        if self.class_att_w is None:
            return None
        
        known_w = self.class_att_w[:, :self.category_num]
        max_weight = known_w.max(dim=-1)[0]
        return torch.tensor(max_weight).to(self.device)
        
    def unknown_prediction(self, cos_sims, batch=10):
        """
            logits (batch, num_patch*num_patch, num_att): cos similarity with visual embeddings
        """  
        if self.distributions is None:
            return cos_sims.max(dim=-1, keepdim=True)[0]
        class_agnotic_confidence = cos_sims   # (batch, num_patch*num_patch, num_known)
        class_agnotic_confidence = class_agnotic_confidence @ self.unknown_att_w
        
        indices = ((cos_sims) // self.mean_interval).long()
        if indices.max() >= self.distributions.shape[-1]:
            indices = indices.clamp(max=self.distributions.shape[-1]-1)
        num_att = indices.shape[-1]
        mean_y = [self.distributions[att_i][indices[..., att_i]] for att_i in range(num_att)]
        mean_y = torch.stack(mean_y, dim=-1)
        
        unknown_logits = ((mean_y @ self.unknown_att_w))
        
        return ((unknown_logits*self.alpha + (1-self.alpha)*class_agnotic_confidence))[..., None]
